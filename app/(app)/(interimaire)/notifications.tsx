...
interface UnifiedNotification {
  ...
  priority: string;
  action_url?: string;
  source: 'candidat' | 'interimaire'; // Pour distinguer la source
}

export default function InterimNotificationsScreen() {
  const { user } = useAuth();
  ...
  const loadNotifications = useCallback(async (pageNum = 1, isRefresh = false) => {
    try {
      ...
      // Charger les deux types de notifications en parallèle
      const [candidatResponse, interimResponse] = await Promise.allSettled([
        getCandidatNotifications(pageNum, 10), // Limiter à 10 pour chaque type
        getNotifications({ page: pageNum, limit: 10 }) // Passer un objet de paramètres
      ]);

      const allNotifications: UnifiedNotification[] = [];

      // Ajouter les notifications candidat
      if (candidatResponse.status === 'fulfilled' && candidatResponse.value?.success && candidatResponse.value?.notifications) {
        const candidatNotifs = candidatResponse.value.notifications.map((notif: any) => ({
          ...notif,
          source: 'candidat' as const
        }));
        allNotifications.push(...candidatNotifs);
      }

      // Ajouter les notifications intérimaire (structure différente : data.notifications)
      if (interimResponse.status === 'fulfilled' && interimResponse.value?.success && interimResponse.value?.data?.notifications) {
        const interimNotifs = interimResponse.value.data.notifications.map((notif: any) => ({
          ...notif,
          source: 'interimaire' as const
        }));
        allNotifications.push(...interimNotifs);
      }

      // Debug: Log des réponses pour comprendre la structure
      console.log('🔍 DEBUG - Candidat Response:', candidatResponse.status === 'fulfilled' ? candidatResponse.value : candidatResponse.reason);
      console.log('🔍 DEBUG - Interim Response:', interimResponse.status === 'fulfilled' ? interimResponse.value : interimResponse.reason);
  ...
  const handleMarkAsRead = async (notification: UnifiedNotification) => {
    ...
    setNotifications(prev => prev.map(n =>
      n.id === notification.id && n.source === notification.source ? { ...n, read_at: new Date().toISOString() } : n
    ));
    try {
      if (notification.source === 'candidat') {
        await markCandidatNotificationAsRead(notification.id);
      } else {
        await markNotificationAsRead(notification.id);
      }

      // Rafraîchir le compteur après succès de l'API avec un petit délai
      setTimeout(() => {
        refreshUnreadCount();
      }, 100);
    } catch (error) {
      ...
      setNotifications(prev => prev.map(n =>
        n.id === notification.id && n.source === notification.source ? { ...n, read_at: undefined } : n
      ));
      refreshUnreadCount();
    }
  };
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      // Marquer toutes les notifications des deux types en parallèle
      ...
      // Revenir en arrière
      setNotifications(prev => prev.map(n => {
        const wasUnread = unreadNotifications.find(un => un.id === n.id && un.source === n.source);
        ...
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              let response;
              if (notification.source === 'candidat') {
                response = await deleteCandidatNotification(notification.id);
              } else {
                response = await deleteNotification(notification.id);
              }

              if (response.success) {
                setNotifications(prev => prev.filter(n => !(n.id === notification.id && n.source === notification.source)));
  ...
  const handleNotificationPress = (notification: UnifiedNotification) => {
    ...
    if (notification.action_url) {
      ...
      // Redirection par défaut selon le type - incluant les types intérimaires
      switch (notification.type) {
        ...
        case 'offre_matching':
          ...
          break;
        // Types spécifiques à l'intérimaire
        case 'ipm_validation':
        case 'feuille_soins_approved':
        case 'feuille_soins_rejected':
        case 'mission_assigned':
        case 'mission_completed':
          router.push('/(app)/(interimaire)/e-interimaire');
          break;
        case 'structure_news':
          router.push('/(app)/(interimaire)/structures');
          break;
        default:
  ...
  const getNotificationIcon = (type: string, priority: string) => {
    ...
    switch (type) {
      ...
      // Icônes spécifiques à l'intérimaire
      case 'ipm_validation':
        return <Ionicons name="document-text" size={24} color="#10b981" />;
      case 'feuille_soins_approved':
        return <Ionicons name="medical" size={24} color="#10b981" />;
      case 'feuille_soins_rejected':
        return <Ionicons name="medical-outline" size={24} color="#ef4444" />;
      case 'mission_assigned':
        return <Ionicons name="briefcase" size={24} color="#3b82f6" />;
      case 'mission_completed':
        return <Ionicons name="checkmark-done" size={24} color="#10b981" />;
      case 'structure_news':
          ...
          <View style={styles.textContainer}>
            ...
            <View style={styles.metaRow}>
              ...
              <View style={styles.badgesContainer}>
                <View style={[styles.sourceBadge, item.source === 'candidat' ? styles.candidatBadge : styles.interimaireBadge]}>
                  <Text style={styles.sourceText}>
                    {item.source === 'candidat' ? 'Candidat' : 'Intérimaire'}
                  </Text>
                </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) => item.id?.toString() || `interim-notification-${index}`}
          refreshControl={
